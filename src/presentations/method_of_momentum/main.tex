...
%    }%
	...
	\frame[c]{
		\begin{adjustwidth}{-2em}{-2em}
		The gradient of $f$ is said to be \emph{Lipschitz continuous} if there 
	...
	\frame[c]{
		...
		\[\exists\underline{m}>0, H(\cdot)\succeq \underline{m}I\;,\]
...
% \end{bmatrix}(x)\]
		...
		\begin{tcolorbox}
			\begin{enumerate}
				\item Definitions:
					...
					$\varepsilon_x$, $\varepsilon_f$ be the \emph{tolerances} 
					and $x(0)=x_0\in D$ be an initial point
				...
				If $|x(k) - x^\ast|\leq\varepsilon_x$ and/or $|f(x(k)) - f(x^\ast)|\leq\varepsilon_f$, stop. Else, go to 2.
			\end{enumerate}
		\end{tcolorbox}
		\end{adjustwidth}
	...
	\frame[c]{
		...
		Consequence: let $\varepsilon>0$ be thenote error tolerance the minimum number of iterations $n$ is $O(1/\varepsilon)$
	...
	\frame[c]{
		...
		\begin{itemize}
		\item A point $\nu$ is said to be an \emph{equilibrium point}
		to $v^+=Rv$ if $R\nu=\nu$

		\item  The equilibrium $\nu$ is said to be (Lyapunov) \emph{stable}
		if
		\[\forall\varepsilon>0, \forall v, \exists\delta>0:|v-\nu|<\delta\Rightarrow
		|V(k,v)-V(k,\nu)|<\varepsilon, \forall k\in\mathbb{N}\]

		\item The equilibrium $\nu$ is said to be \emph{attractive} if
		\[\exists\delta>0, \forall |v-\nu|<\delta, \lim_{k\to\infty} V(k,v)\to \nu\]

		\item The equilibrium $\nu$ is said to be \emph{asymptotically stable}
	...
	\frame[c]{
		...
		Asymptotically stable if eigenvalues of $R$ satisfy $\max\{|\sigma_1|, 
		|\sigma_2|\} <1$

		Consequently, $\alpha$ and $\beta$ must satisfy the inequalities
