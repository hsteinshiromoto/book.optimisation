...
% \end{bmatrix}(x)\]
	...
	\frame[c]{
		...
		\begin{tcolorbox}
			\begin{enumerate}
				...
				If $|f(x(k)) - f(x^\ast)|\leq\varepsilon_f$, stop. Else, go to 2.
			\end{enumerate}
		\end{tcolorbox}
		\end{adjustwidth}
	...
	\frame[c]{
		...
		Limits on convergence:
			\begin{tcolorbox}[title=Theorem]
				For every integer $k\leq(n-1)/2$ and for every $x(0)\in\dom f$,
	...
	\frame[c]{
		...
		\begin{adjustwidth}{-2em}{-2em}
		\begin{figure}[htpb!]
			\includegraphics[width=\linewidth]{imgs/mom_gd_mom_comp.eps}
		\end{figure}
		\end{adjustwidth}
	}
