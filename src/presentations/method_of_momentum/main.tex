...
%    }%
	...
	\frame[c]{
		...
		Let $D\subset\mathbb{R}^n$ be an open set, and $f\in\mathcal{C}^1(\mathbb{R}^n,\mathbb{R})$
	...
	\frame[c]{
		\frametitle{Gradient Descent: Hypothesis}
	...
	\frame[c]{
		...
		\begin{tcolorbox}
			\begin{enumerate}
				\item Let $k=0$, $\alpha>0$ be the \emph{stepsize}, and $x_0\in D$ be an initial point
		...
		\begin{tcolorbox}[title=Theorem]
				Let $L$ be the Lipschitz constant of $\nabla f$ and $\alpha\leq 1/L$, then for every $k\in\mathbb{N}_{>0}$
	...
	\frame[c]{
		...
		\begin{tcolorbox}[title=Lemma]
				If $f$ be Lipschitz on $D$ with constant $L$, then for every $x,y\in D$,
				 \[|f(x)-f(y)-\nabla f(x)\cdot(y-x)|\leq\dfrac{L}{2}|x-y|^2\]
		\end{tcolorbox}

		\begin{itemize}
			\item Let $x_{k+1}=x_k-\alpha \nabla f(x_k)$. From the previous lemma, 
			\begin{align*}
